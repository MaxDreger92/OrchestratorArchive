...
3. The unit needs to match the property for this you can use the header content as well as your knowledge in materials science. A unitless property should have an empty string "" as unit.
...
MATTER_AGGREGATION_MESSAGE = [
    ...
    Your input will be:
    Context:
        - Additional information about the data or domain
    Full Table:
        - headers
        - first row
    Data:
...
Your task is to use this information to correctly transform the given data list into a list of matter node(s). The final 
list should contain all distinguishable matte(r) nodes and their attributes. Use every element of the list to generate the nodes you have to 
...
Thought 4: The header of the tables can contain additional descriptions of materials that you should add to the name list (only if it is a specific name/description). 
Thought 5: Never store components and its parts in one and the same node.
Thought 6: Measurement data often contains only a single Matter node while fabrication workflows usually contain multiple matter nodes
...
1. Each Attribute consists of:
    ...
    AttributeReference: Either the ColumnIndex if extracted from the table cell or "header" or "guess" if the attribute was inferred from the table header or the context.
2. Always follow the requested format!
3. The name of the Measurement can be inferred from the context you get or by the headers of the table. The name should only contain the specific name of the measurement method.
4. Inferred attributes have a AttributeReference that is either "header" or "guess" depending on the source of the information.
    """),
    ("human",
     """Use the given format to extract the properties and their attributes from the following table: {input}"""),
 ]
...
1. Each Attribute consists of:
    ...
    AttributeReference: Either the ColumnIndex if extracted from the table cell or "header" or "guess" if the attribute was inferred from the table header or the context.
2. Always follow the requested format!
3. The metadata_type can be inferred from the headers of the table. 
4. The value usually be extracTed from the table cell.
    """),
    ("human", """Use the given format to extract the properties and their attributes from the following table: {input}"""),
 ]

SIMULATION_AGGREGATION_MESSAGE = [
    ("system", """You are a world-class knowledge graph generating algorithm to extract nodes from materials science tables.
...
Your only job is to extract a list of simulations and their attributes from the data.
You perfectly extract the different simulations by agglomerating the data from the table correctly using your vast knowledge in materials science.
REMEMBER:
1. Each Attribute consists of:
    AttributeValue: Value of the attribute(can be extracted from the table cell or the header, or the context)
    AttributeReference: Either the ColumnIndex if extracted from the table cell or "header" or "guess" if the attribute was inferred from the table header or the context.
2. Always follow the requested format!
3. The name of the Measurement can be inferred from the context you get or by the headers of the table. The name should only contain the specific name of the simulation method.
4. Inferred attributes have a AttributeReference that is either "header" or "guess" depending on the source of the information.
    """),
    ("human",
     """Use the given format to extract the properties and their attributes from the following table: {input}"""),
